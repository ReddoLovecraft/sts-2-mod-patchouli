...
[Tool]
public partial class NPatchouliFlyToTarget : Node2D
{
	[Export] public bool Enabled { get; set; } = true;
	[Export] public bool AutoStart { get; set; } = true;
	[Export] public Vector2 StartOffset { get; set; } = new Vector2(-240f, 0f);
	[Export] public float Speed { get; set; } = 1200f;
	[Export] public float CurveAmount { get; set; } = 0f;
	[Export] public float AutoFreeSeconds { get; set; } = 0.8f;
	[Export] public bool PreviewInEditor { get; set; } = true;
	[Export] public Vector2 EditorPreviewTargetOffset { get; set; } = new Vector2(360f, 0f);
	...
	private float _minAutoFreeSeconds;
	...
	{
		_sprite = GetNodeOrNull<AnimatedSprite2D>("%Sprite") ?? GetNodeOrNull<AnimatedSprite2D>("Sprite");
		if (_sprite != null)
		{
			_sprite.SpeedScale = 1f;
			if (_sprite.SpriteFrames != null && _sprite.SpriteFrames.HasAnimation("default"))
			{
				_sprite.SpriteFrames.SetAnimationLoop("default", false);
				int frames = _sprite.SpriteFrames.GetFrameCount("default");
				float speed = (float)_sprite.SpriteFrames.GetAnimationSpeed("default");
				if (frames > 0 && speed > 0.001f)
				{
					_minAutoFreeSeconds = (frames / speed) + 0.05f;
				}
			}
	...
	public override void _Process(double delta)
	{
		...
		if (!Engine.IsEditorHint() && _finished && _animFinished)
		{
			QueueFree();
			return;
		}

		float safety = Mathf.Max(AutoFreeSeconds, _minAutoFreeSeconds);
		if (!Engine.IsEditorHint() && safety > 0f && _finished && _life >= safety)
		{
			QueueFree();
		}
	}
