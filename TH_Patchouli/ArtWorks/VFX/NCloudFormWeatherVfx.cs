...
public partial class NCloudFormWeatherVfx : Node2D
{
	...
	[Export]
	public float AngularSpeed { get; set; } = 0.8f;
	...
	public override void _Process(double delta)
	{
		...
		Creature? owner = null;
		CloudFormPower? cloudForm = null;
		if (Power is CloudFormPower bound)
		{
			cloudForm = bound;
			owner = bound.Owner;
		}
		owner ??= TryResolveOwnerCreature();
		...
		if (cloudForm == null)
		{
			...
			{
				if (owner.Powers[i] is CloudFormPower found)
				{
					cloudForm = found;
					break;
				}
		...
		Visible = drawPile.Cards.Count > 0;
